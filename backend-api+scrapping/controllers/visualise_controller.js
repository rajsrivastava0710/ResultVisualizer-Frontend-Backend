  ...
  module.exports.getListWithBranchAndAveragePercent = async (req, res, next) => {
    ...
    //   return res.status(200).json(students)
      ...
      students.forEach(student => {
        if(!isNaN(student.percent)) {
            ...
            if(branchData[student.branch] == undefined) {
                branchData[student.branch] = [Number(student.percent), Number(payload)]
            } else {
      ...
      for (var branch in branchData){
        ...
        resArray.push([branch,String(avgPercent),String(passingCount),String(branchStudentCount[branch])])
      }
      
      resArray[0] = ["Branch", "Average Percent", "Students Passed", "Total Students"]
